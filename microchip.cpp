...
public:
  ...
  Microchip(int length, const std::vector<Point> &points) : length(length), points(points), n(points.size()), result_length(0) {
  ...
  void FindDirection(Point &point) {
    ...
    for (const auto &processed_point : processed_points) {
      if (dirs[0]) {
        dirs[0] = intersect(point, {point.x, length - 1, 0, 0, 0}, processed_point,
                            {processed_point.x_end, processed_point.y_end, 0, 0, 0});
      }

      if (dirs[1]) {
        dirs[1] = intersect(point, {length - 1, point.y, 0, 0, 0}, processed_point,
                            {processed_point.x_end, processed_point.y_end, 0, 0, 0});
      }

      if (dirs[2]) {
        dirs[2] = intersect(point, {point.x, 0, 0, 0, 0}, processed_point,
                            {processed_point.x_end, processed_point.y_end, 0, 0, 0});
      }

      if (dirs[3]) {
        dirs[3] = intersect(point, {0, point.y, 0, 0, 0}, processed_point,
                            {processed_point.x_end, processed_point.y_end, 0, 0, 0});
      }
    }

    std::vector<int> lines = {length - point.y - 1, length - point.x - 1, point.y, point.x};
    ...
    switch (min_ind) {
      case 0: {
        point.x_end = point.x;
        point.y_end = length - 1;
        result_length += length - point.y - 1;
        break;
      }
      case 1: {
        point.x_end = length - 1;
        point.y_end = point.y;
        result_length += length - point.x - 1;
        break;
      }
      case 2: {
        point.x_end = point.x;
  ...
  void FindRoutes() {
    ...
    std::vector<std::vector<Point>> grid(n, std::vector<Point>(n, {-1, -1, -1}));
...
//    };
    ...
    for (const auto &point : points) {
      grid[point.x - 1][point.y - 1] = {point.x, point.y, -1};
    }
    ...
    for (int i = 0; i < n / 2; ++i) {
      ...
      for (int j = i; j < n - i; ++j) {
//        std::cerr << grid[j][i] << " ";
        if (grid[j][i].direction != -1) {
          FindDirection(grid[n - j - 1][n - i - 1]);
        }
      }

      for (int j = i + 1; j < n - i; ++j) {
//        std::cerr << grid[n - i - 1][j] << " ";
        if (grid[n - i - 1][j].direction != -1) {
          FindDirection(grid[n - j - 1][n - i - 1]);
        }
      }
...
//        std::cerr << grid[n - j - 1][n - i - 1] << " ";
        if (grid[n - j - 1][n - i - 1].direction != -1) {
          FindDirection(grid[n - j - 1][n - i - 1]);
        }
      }
